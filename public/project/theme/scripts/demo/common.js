...
{
	if (!$('.container:first').is('.menu-hidden') && $('#menu').is('.hidden-phone'))
...
function genSparklines()
{
	if ($('.sparkline').length)
	{
		...
		$.each($('#menu .sparkline'), function(k)
		{
			var size = { w: 150, h: 20 };
...
$(function()
{
	...
	{
		/*
		if (!$('#menu').is(':visible') && !$('.container:first').is('.menu-hidden') && !$('.container:first').is('.documentation') && !$('.container:first').is('.login'))
		{
		...
		else
		{
			...
			if (!$('#menu').is(':visible') && parseInt($(document).width()) > 767 && !$('.container:first').is('.menu-hidden'))
			{
	...
	{
		$('.container:first').toggleClass('menu-right');
		
		if ($(this).prop('checked')) 
			$('.container:first').removeClass('menu-left');
		else
			$('.container:first').addClass('menu-left');
	...
	if (typeof $.cookie != 'undefined' && $.cookie('menuHidden') && $.cookie('menuHidden') == 'true' || (!$('.container').is('.menu-hidden') && !$('#menu').is(':visible')))
		toggleMenuHidden();
