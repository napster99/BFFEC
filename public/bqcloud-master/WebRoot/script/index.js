...
$('#putNote .hd a, #login .hd a, #register .hd a').click(function(){
	...
	$('#register').animate({'top':'-350px'},400,function(){
		$('#mask').hide();
	});
...
$('#quickLogin').click(function(){
	...
	$('#login').animate({'top':'100px'},400);
});

$('a[name=register]').click(function(){
	$('#mask').height($(document).height()).show();
	$('#register').animate({'top':'50px'},400);
});
...
function regCheck(){
	var retValue = true;
	if($.trim($('#register').find('input[name=pwd]').val()) == ''){
		$('#register').find('input[name=pwd]').addClass('errorstyle');
		retValue = false;
	}
	
	if($.trim($('#register').find('input[name=account]').val())== ''){
		$('#register').find('input[name=account]').addClass('errorstyle');
		retValue = false;
	}
	
	if($.trim($('#register').find('input[name=nickname]').val())== ''){
		$('#register').find('input[name=nickname]').addClass('errorstyle');
		retValue = false;
	}
	return retValue;
}
...
function resetPub(){
	 ...
	 $('#putNote').find('input[name=nickname]').val(nickname);
}

function resetReg(){
	$('.regError').hide();
	$('#register').find('input[name=account]').val('').removeClass('errorstyle');
	$('#register').find('input[name=pwd]').val('').removeClass('errorstyle');
	$('#register').find('input[name=nickname]').val('').removeClass('errorstyle');
}
 ...
 * @date 	   : 2013-03-12 
 */
function login(act,password){
//	$('#loginForm').submit();
	var account = $('#loginForm').find('input[name=account]').val() || '',
		pwd =  $('#loginForm').find('input[name=pwd]').val() || '';
 ...
 */
function register(){
	var account = $('#regForm').find('input[name=account]').val() || '',
		pwd = $('#regForm').find('input[name=pwd]').val() || '',
	...
	try{
		$.ajax({
			url : 'Person.do',
			type : 'post',
			data : {'account':account ,'pwd':pwd, 'nickname':nickname, 'method':'register'},
			dataType : 'text',
			success : function(data){
				if(data == 'success'){
					login(account,pwd);
